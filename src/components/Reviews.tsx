  ...
  return (
    <div ref={containerRef} className="w-full">
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        {loading ? (
          <div className="lg:col-span-4 liquid-glass rounded-3xl p-8 text-center flex flex-col justify-center items-center space-y-6 animate-pulse">
            {/* Confianza Label */}
            <div className="h-4 w-20 bg-white/[0.08] dark:bg-white/[0.08] light:bg-black/[0.06] rounded mx-auto"></div>
            
            {/* Score and stars */}
            <div className="space-y-1">
              <div className="h-16 w-24 bg-white/[0.08] dark:bg-white/[0.08] light:bg-black/[0.05] rounded-2xl mx-auto"></div>
              <div className="flex items-center justify-center gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="w-9 h-9 rounded-full bg-white/[0.06] dark:bg-white/[0.06] light:bg-black/[0.05]" />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="h-5 w-48 bg-white/[0.06] dark:bg-white/[0.06] light:bg-black/[0.05] rounded mx-auto"></div>
              <div className="h-3 w-32 bg-white/[0.04] dark:bg-white/[0.04] light:bg-black/[0.03] rounded mx-auto"></div>
            </div>
          ...
          <div className={`card-hover lg:col-span-4 liquid-glass rounded-3xl p-8 text-center flex flex-col justify-center items-center space-y-6 review-reveal-item ${revealed ? 'is-visible' : ''}`}>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-center gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <svg
                    key={i}
                    className={`w-9 h-9 ${i <= Math.round(rating) ? 'text-amber-400' : 'text-white/10'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-[hsl(var(--fg))]">
                {`${count} opiniones en Google Maps`}
              </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="h-2.5 w-14 bg-white/[0.04] dark:bg-white/[0.04] light:bg-black/[0.03] rounded"></div>
                      </div>
                    </div>
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <div key={star} className="w-4 h-4 rounded-full bg-white/[0.06] dark:bg-white/[0.06] light:bg-black/[0.05]"></div>
                      ))}
                    </div>
                  </div>
                  <div className="space-y-2 pt-2">
                    <div className="h-3 w-full bg-white/[0.04] dark:bg-white/[0.04] light:bg-black/[0.03] rounded"></div>
                    <div className="h-3 w-11/12 bg-white/[0.04] dark:bg-white/[0.04] light:bg-black/[0.03] rounded"></div>
                    <div className="h-3 w-2/3 bg-white/[0.04] dark:bg-white/[0.04] light:bg-black/[0.03] rounded"></div>
                  </div>
                </div>
              </div>
            ))
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <svg
                          key={star}
                          className={`w-4 h-4 ${star <= r.rating ? 'text-amber-400' : 'text-white/10'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
